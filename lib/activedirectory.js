...
/**
 * Performs task of adding user to group
 * 
 * @private
 * @param {String} uDn user's distinguished name
 * @param {String} gDN group's distinguished name
 * @param {Function} [callback] The callback to execute when complete. callback(err: {Object})
 */
 function addToGroup(uDn,gDn,callback){
  var client = createClient.call(self,null,null);
  ...
  client.modify(gDn,change,function(err,res){
    if (err) {
      if (callback) callback(err);
      return;
    } 
...
/**
 * Performs task  of removing the user from a group
 * 
 * @private 
 * @param {String} uDn user's distinguished name
 * @param {String} gDn groups's distinguished name
 * @param {Function} [callback] The callback to execute when complete. callback(err: {Object})
 */
function removeFromGroup(uDn,gDn,callback){
  var client = createClient.call(self,null,null);
  var change = new ldap.change({
    operation: 'del',
    modification :{
 ...
 * 
 * @public
 * @param {Object} [] Optional LDAP query string parameters to execute { scope: '',filter: '', attributes: [ '', '', ... ], sizeLimit: 0, timelimt: 0}
 * @param {String} userName Username to remove from group
 * @param {String} groupName Groupname to remove from.
 * @param {Function} [callback] The callback to execute when complete. callback(err: {Object})
 */

ActiveDirectory.prototype.removeUserFromGroup = function removeUserFromGroup(opts,userName,groupName,callback){
 var self = this;
 ...
 log.trace('removeUserFromGroup(%j,%s,%s)',opts,userName,groupName);

  getGroupDistinguishedName.call(self,opts,groupName,function(err,gDn){
    if (err) {
      if (callback) callback(err);
      return;
    }
    ...
    getUserDistinguishedName.call(self, opts, username, function(err,uDN){
      if(err){
        if (callback) callback();
        return;
      }
      ...
      if(!udn){
        log.warn ('Could not find a distinguishedName for the specified user name : "%s"', userName);
        if (callback) callback();
        return;
      }
      removeFromGroup(uDn,Gdn,function(err,res){
        if (err){
          if (callback) callback(err);
          return;
        }
 ...
 * 
 * @public
 * @param {Object} [] Optional LDAP query string parameters to execute. { scope: '', filter: '', attributes: [ '', '', ... ], sizeLimit: 0, timelimit: 0 }
 * @param {String} username Username to add to group.
 * @param {String} groupname Name of Group to add user too.
 * @param {Function} [callback] The callback to execute when complete. callback(err: {Object})
 */
ActiveDirectory.prototype.addUserToGroup = function addUserToGroup(opts,username,groupname,callback){
  var self = this;
  ...
  log.trace('addUserToGroup(%j,%s,%s)',opts,userName,groupName);
  ...
  getGroupDistinguishedName.call(self,opts,groupName,function(err,gDn){
    ...
    getUserDistinguishedName.call(self, opts, username, function(err,uDN){
      ...
      addToGroup(uDn,Gdn,function(err,res){
        if (err){
          if (callback) callback(err);
          return;
        }
